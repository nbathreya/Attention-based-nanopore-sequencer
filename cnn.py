...
def conv_layer(indata, ksize, padding, training, name, dilate=1, strides=None, bias_term=False, active=True,
               BN=True, active_function='swish',wd = None):
    ...
    if active:
        ...
        elif active_function == 'swish':
            with tf.variable_scope(name + '_swish'):
                conv_out = 1.67653251702*conv_out*tf.nn.sigmoid(conv_out, name='swish')
    return conv_out
...
#def main():
    ...
    #cnn_feature = getcnnfeature(tf.convert_to_tensor(inputX), training=training)
    #print(cnn_feature)
