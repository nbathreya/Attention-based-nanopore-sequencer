...
from tensorflow.contrib.rnn import SRUCell
...
def rnn_layers(x,
    ...
    attention_mechanism = tf.contrib.seq2seq.LuongAttention(
        hidden_num, x, memory_sequence_length=seq_length)
    ...
    decoder_cell_fw = tf.contrib.seq2seq.AttentionWrapper(
        multi_cells_fw, attention_mechanism, attention_layer_size=hidden_num)

    decoder_cell_bw = tf.contrib.seq2seq.AttentionWrapper(
        multi_cells_bw, attention_mechanism, attention_layer_size=hidden_num)

    with tf.variable_scope('BDGRU_rnn') as scope:
        outputs, _ = tf.nn.bidirectional_dynamic_rnn(
                cell_fw=decoder_cell_fw, cell_bw=decoder_cell_bw, inputs=x, sequence_length=seq_length, dtype=dtype, scope=scope)
    ...
    with tf.variable_scope('rnn_fnn_layer'):
        ...
        bias_class = _variable_on_cpu(name='bias_class',
                                      ...
                                      dtype = dtype)
        '''
        weight_out = _variable_on_cpu(name='weights',shape = [2, hidden_num],dtype = dtype)
        biases_out = _variable_on_cpu(name='bias',shape = [hidden_num],dtype = dtype)
        weight_class = _variable_on_cpu(name='weights_class',shape = [hidden_num, class_n],dtype = dtype)
        bias_class = _variable_on_cpu(name='bias_class',shape = [class_n],dtype = dtype)
        '''
...
def rnn_layers_one_direction(x, seq_length, training, hidden_num=200, layer_num=3, class_n=5,dtype = tf.float32):
    ...
    decoder_cell = tf.contrib.seq2seq.AttentionWrapper(
        cell_wrap, attention_mechanism, attention_layer_size=hidden_num)
    with tf.variable_scope('LSTM_rnn') as scope:
        lasth, _ = tf.nn.dynamic_rnn(
            decoder_cell, x, sequence_length=seq_length, dtype=tf.float32, scope=scope)
    # shape of lasth [batch_size,max_time,hidden_num*2]
