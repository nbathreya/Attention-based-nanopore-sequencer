...
sys.path.append('C:/Users/nbath/Downloads/CS598-JP/project')

raw_labels = collections.namedtuple('raw_labels', ['start', 'length', 'base'])

class Flags():
    def __init__(self):
        self.home_dir = "/home/athreya/Dropbox/CS598-JP/project"
        self.data_dir = '/home/athreya/Dropbox/CS598-JP/project/train/'
        self.log_dir = '/home/athreya/Dropbox/CS598-JP/project/log/'
        self.sequence_len = 400
        self.batch_size = 400
        self.step_rate = 1e-2
        self.max_steps = 10000
        self.model_name = 'read_test'
...
def padding(x, L, padding_list=None):
    ...
    if padding_list is None:
        ...
        x.extend(padding_list + [-1] * (zero_n - len(padding_list)))
    else:
...
def read_raw_data_sets(data_dir, h5py_file_path=None, seq_length=FLAGS.sequence_len, k_mer=1, max_segments_num=FLAGS.max_segments_number):
    ###Read from raw data
...
def read_data_for_eval(file_path, start_index=0, step=20, seg_length=400, sig_norm=True):
    """
    Input Args:
...
def main():
    ### Input Test ###
    #Data_dir = '/home/athreya/Downloads/train/'
    h5py_file_path = '/home/conjugacy/Downloads/project/h5py_file_path'
    #train = read_raw_data_sets(FLAGS.data_dir, FLAGS.sequence_len)
    train = read_cache_dataset(h5py_file_path)
    for i in range(10):
        inputX, sequence_length, label, label_att = train.next_batch(10)
    #print("train:", train)
    #print("inputX:", inputX)
    #print("seq_length:", sequence_length)
    #print("label:", label)
    indxs, values, shape = label
    print(indxs[10])
    print(values[10])
    #print("label:", label)
    print("label_att:", label_att[1][0].shape)
